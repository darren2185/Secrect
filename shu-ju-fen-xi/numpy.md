...
b[-1]   # array([30, 31, 32, 33]) is equivalent with b[-1, : ]
...
若想逐一访问每个元素，建议使用flat函数，其能展平所有元素成为一个一维数组
...
* 浅复制：view函数创建一个新数组，其可以改变任何shape的数组，但如果一方内部元素发生更改，另外一方也会跟着更改。
* 深度复制（即一方元素发生更改，不影响另外一方）copy函数

![](/assets/numpy_function.png)
